...
Multiservo::Error Multiservo::detach()
{
    ...
    Error err = writeMicroseconds(m_iPin, 0, 255, m_twiAddress, nAttemptsDefault);
...
Multiservo::Error Multiservo::write(int value, int speed)
{
    if (value < m_minPulse) {
    ...
    return writeMicroseconds(value, speed);
}

Multiservo::Error Multiservo::writeMicroseconds(int pulseWidth, int speed)
{
    ...
    return writeMicroseconds(m_iPin, m_pulseWidth, speed, m_twiAddress, nAttemptsDefault);
}
...
bool Multiservo::isMoving()
{
    ...
    Wire.requestFrom(m_twiAddress, 1);    // request 6 bytes from slave device #8
